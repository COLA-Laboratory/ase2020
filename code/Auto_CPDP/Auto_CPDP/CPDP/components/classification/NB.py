...
class NB(AutoClassifier):
    def __init__(self, NBType='gaussian', alpha=1.0, norm=False):
        self.alpha = alpha
        self.NBType = NBType
        self.estimator = None
        self.norm = norm
    ...
    def fit(self, X, y, sample_weight):
        if self.NBType == 'gaussian':
            ...
            self.estimator = sklearn.naive_bayes.ComplementNB(alpha=self.alpha, norm=self.norm)
    ...
    def get_hyperparameter_search_space(dataset_properties=None):
        ...
        Type = CategoricalHyperparameter(name="NBType",
                                         choices=["gaussian", 'multinomial', 'complement'],
                                         default_value="gaussian")
        alpha = UniformFloatHyperparameter('alpha', 0, 10, default_value=1.0)
        norm = CategoricalHyperparameter('norm', [True, False], default_value=False)

        cs.add_hyperparameters([Type, alpha, norm])
        cond1 = EqualsCondition(norm, Type, 'complement')
        cond2 = InCondition(alpha, Type, ['multinomial', 'complement'])
