    ...
    def run(self):
        for i in range(10):
            ...
            try:
                ...
                with open(self.dir + '/' + str(time.time()) + 'Time20.txt', 'w') as f:
...
class Ulevel(object):
    ...
    def run(self):
        ...
        if self.method == 'paratabu':
            exc = paraTabu_multi_start(f=self.f, range=self.paramRange, dir=self.ufname, max=self.FE, stime=self.startTime)
            try:
                fres = exc.run()
...
def bl(Xsource, Lsource, Xtarget, Ltarget, loc, fname, method, repeat=10):
    ...
    for i in range(repeat):
        ...
        ex = Ulevel(params, xsource=Xsource, ysource=Lsource, xtarget=Xtarget, ytarget=Ltarget, loc=loc,
                    UFE=208, LFE=10000, method=method, fname=fname)
        res, inc = ex.run()
        path = create_dir('resBL')
        with open(path + fname + '-Time20' + '.txt', 'a+') as f:
            print(inc, file=f)
            print(res, file=f)
            print('---------------------', file=f)
